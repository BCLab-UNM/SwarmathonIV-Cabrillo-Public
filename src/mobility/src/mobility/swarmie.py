...
class Swarmie: 
    ...
    def sees_resource(self, required_matches,crop=True):
        ...
        * (`bool`): True if the number of required matches has been met, False otherwise.
        ...
        import pickle
        img2 = pickle.load( open(rospy.get_param('/achilles/mobility/april_tag_resource_pickel'), "rb" ) ) #fix this for all rovers
        if(crop):       #(y1:y2, x1:x2)
        ...
        sift = cv2.xfeatures2d.SIFT_create() #may need `pip install opencv-contrib-python`
